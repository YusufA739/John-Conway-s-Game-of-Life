...
def rainlogic(currentframelengthofraindrop, linesperframe, cellsperline, lowestraindropnumber, highestraindropnumber,
              lowestvalue, stopvalue, highestvalue):
    ...
    for i in range(cellsperline):  # same as len(lengthofraindropNextLine)
        if lengthofraindropNextLine[i] > stopvalue:
            if highestraindropnumber > lowestraindropnumber:
                nextline += str(
                    random.randint(lowestraindropnumber, highestraindropnumber))  # change to (0,1) for binary
...
def setup(linesperframe, cellsperline, framebufferMaxSize, lowestraindropnumber, highestraindropnumber, lowestvalue,
          highestvalue):
    frameCount = 0
    framebuffer = []
    liveGrid = []
        ...
        for line in information:
            ...
            if len(livelines) != cellsperline:
                for carrier in range(cellsperline - len(livelines)):
                    livelines.append(
                        0)  # pad it out extra, otherwise we will get errors when it tries to render a pixel
                    # that doesn't exist
...
while True:
    ...
    for carrier in range(0, framebufferMaxSize - 2, 1):
        if framebuffer[carrier] == framebuffer[carrier + 2]:
            frameCount, framebuffer, liveGrid, currentframe = setup(linesperframe, cellsperline, framebufferMaxSize,
                                                                    lowestraindropnumber, highestraindropnumber,
